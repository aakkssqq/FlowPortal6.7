...
public class YZProductionDeviceServices : YZServiceHandler
{
    ...
    public JObject GetData(HttpContext context)
    {
        ...
        string query = @"
            WITH X AS(
                SELECT ROW_NUMBER() OVER(ORDER BY {0}) AS RowNum,* FROM 客户档案 {1}
            ),
            Y AS(
    ...
    public JObject GetDataNoRecordPerm(HttpContext context)
    {
        ...
        if (searchType == "QuickSearch")
        {
            //应用关键字过滤
            if (!string.IsNullOrEmpty(keyword))
                filter = queryProvider.CombinCond(filter,String.Format("CompanyName LIKE N'%{0}%' OR Addr LIKE N'%{0}%'", queryProvider.EncodeText(keyword)));
        }
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                using (YZReader reader = new YZReader(cmd.ExecuteReader()))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        item["Addr"] = reader.ReadString("Addr");
                        item["Contact"] = reader.ReadString("Contact");
                        item["Phone"] = reader.ReadString("Phone");
                    }
