...
using YZSoft.Web.Queue;
using BPM.Client.Queue;

namespace YZSoft.Services.REST.Queue
{
    public class ServiceHandler : YZServiceHandler
    {
        public virtual object GetLogSucceed(HttpContext context)
        {
            ...
            using (IYZDbProvider provider = YZDbProviderManager.DefaultProvider)
            {
                ...
                {
                    return WebQueueManager.GetLogSucceed(provider, cn, date, this.GetFilterString(request, provider), request.GetSortString("MessageID DESC"), request.Start, request.Limit);
                }
            }
        }

        public virtual object GetLogFailed(HttpContext context)
        {
            YZRequest request = new YZRequest(context);
            DateTime date = request.GetDateTime("date");
            ...
            using (IYZDbProvider provider = YZDbProviderManager.DefaultProvider)
            {
                using (IDbConnection cn = provider.OpenConnection())
                {
                    return WebQueueManager.GetLogFailed(provider, cn, date, this.GetFilterString(request, provider), request.GetSortString("MessageID DESC"), request.Start, request.Limit);
                }
            }
        }

        public virtual object GetQueueMessages(HttpContext context)
        {
            YZRequest request = new YZRequest(context);
            ...
            using (IYZDbProvider provider = YZDbProviderManager.DefaultProvider)
            {
                using (IDbConnection cn = provider.OpenConnection())
                {
                    return WebQueueManager.GetQueueMessages(provider, cn, this.GetFilterString(request, provider), request.GetSortString("MessageID DESC"), request.Start, request.Limit);
                }
            }
        }

        public void RetryFailedMessage(HttpContext context)
        {
            YZRequest request = new YZRequest(context);
            ...
            using(BPMConnection cn = new BPMConnection())
            {
                ...
                QueueManager.RetryFailedMessage(cn, messageId);
            }
        }
