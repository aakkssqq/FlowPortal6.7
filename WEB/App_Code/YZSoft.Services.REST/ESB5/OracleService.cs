...
namespace YZSoft.Services.REST.ESB5
{
    ...
    ///OracleHandler 的摘要说明
    /// </summary>
    public class OracleHandler : YZServiceHandler
    {
        /// <summary>
        /// 连接到服务器
        /// </summary>
        ...
        public JObject GetParameter(HttpContext context)
        {
            ...
            try
            {
                ...
                //查询语句发生改变，重新分析语句
                List<string> isonly = new List<string>();
                ...
                foreach (var item in paramArray)
                {
                    ...
                    if (index != -1)
                    {
                        ...
                        if (isonly.Contains(str))
                        {
                            obj[YZJsonProperty.success] = false;
                            obj[YZJsonProperty.errorMessage] = string.Format("查询语句中包含了重复的参数命名：{0}", str);
                            return obj;
                        }
                        else
                        {
